...
func setUpReliabilityParamTest(t *testing.T, options *RTCDataChannelInit) (*RTCPeerConnection, *RTCPeerConnection, *RTCDataChannel, chan bool) {
	api := NewAPI()
	offerPC, answerPC, err := api.newPair()
	if err != nil {
		t.Fatalf("Failed to create a PC pair for testing")
...
func closeReliabilityParamTest(t *testing.T, pc1, pc2 *RTCPeerConnection, done chan bool) {
